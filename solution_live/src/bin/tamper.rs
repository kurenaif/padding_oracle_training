...
#[tokio::main]
async fn main() {
    let mut message = b"{\"id\":9,\"admin\":true}".to_vec();
    let pad_value = 16 - message.len() % 16;
    let pad = vec![pad_value as u8; pad_value];
    message.extend(pad.iter());

    let token = vec![0u8; message.len() + 16];
    ...
    for block in 0..(block_count - 1) {
        'outer: for iter in 0..iter_count {
            ...
            for c in (0u8..=255).rev() {
                ...
                let resp = check(&modified[..len - block * 16]).await;
        ...
        for i in 0..16 {
            let offset = len - 16 - (block * 16) + i;
            let offset_message = message.len() - 16 - (block * 16) + i;
            modified[offset - 16] ^= 16 ^ 1 ^ message[offset_message];
        }
        println!("{:02x?}", modified);
        println!("token: {}", base64::encode_config(&modified, base64::URL_SAFE));
